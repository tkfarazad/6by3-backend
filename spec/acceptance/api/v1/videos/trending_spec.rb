        ...
        context 'user authenticated', :authenticated_user do
          context 'returns all records with' do
            example 'Responds with 200' do
              [video4, video2, video1, video3].each { |video| create(:video_view, user: user1, video: video) }
              [video4, video2, video1].each { |video| create(:video_view, user: user2, video: video) }
              [video4, video2].each { |video| create(:video_view, user: user3, video: video) }
              [video4].each { |video| create(:video_view, user: user4, video: video) }
              ...
              expect(parsed_body[:data][0][:id]).to eq_id video4.id
              expect(parsed_body[:data][1][:id]).to eq_id video2.id
              expect(parsed_body[:data][2][:id]).to eq_id video1.id
              expect(parsed_body[:data][3][:id]).to eq_id video3.id
